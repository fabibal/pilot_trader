#!/usr/bin/env python3
"""
Historical audit of @CryptoFeras (ANALYSIS ONLY).
...
POSTS ONLY: overrides the fetch path to the posts-only "tweets" endpoint
(NOT tweets_and_replies) so @-replies are excluded, per request.
...
ACCOUNT = "CryptoFeras"
OUT_FILE = os.path.join(monitor.HOME, "results", "audit_CryptoFeras.json")
N_TWEETS = 200
...
# POSTS ONLY: use the posts-only "tweets" endpoint, not tweets_and_replies.
monitor.GETXAPI_TWEETS_PATH = "/twitter/user/tweets"
...
def main():
    ...
    print(f"Fetching up to {N_TWEETS} POSTS (no replies) for @{ACCOUNT} ...",
          file=sys.stderr)
    ...
    report = {
        ...
        "note": "ANALYSIS ONLY. Not added to monitored ACCOUNTS. "
                "POSTS ONLY (tweets endpoint, replies excluded). "
                "trades.json / positions.json / state untouched.",
        "endpoint": monitor.GETXAPI_TWEETS_PATH,
