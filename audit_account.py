#!/usr/bin/env python3
"""
Historical audit of one X account (ANALYSIS ONLY).

Usage: python audit_account.py <handle>
...
prices (yfinance), and writes results/audit_<handle>.json.

POSTS ONLY: @-replies are dropped before extraction (both GetXAPI's isReply
flag and a leading-@ text check).

DOES NOT touch trades.json / positions.json / .monitor_state.json, and does
NOT add the handle to monitor.ACCOUNTS.
"""
...
if len(sys.argv) < 2:
    print("Usage: python audit_account.py <handle>", file=sys.stderr)
    sys.exit(1)
...
# Treat the audited handle as an influencer so the Sonnet vision pass runs on
# chart photos (same handling as @IncomeSharks). This only mutates the in-memory
# SOURCE_TYPE map for this process; it does NOT add the handle to ACCOUNTS.
monitor.SOURCE_TYPE[ACCOUNT] = "influencer"
monitor.MAX_FETCH = N_TWEETS


def is_reply_tweet(tw):
    """Posts-only filter: drop @-replies (GetXAPI isReply flag or leading @)."""
...
def main():
    ...
    fetched, calls = monitor.fetch_getxapi(ACCOUNT)   # since_id=None -> newest N
    tweets = [t for t in fetched if not is_reply_tweet(t)]   # posts only
    print(f"  fetched {len(fetched)} tweets in {calls} GetXAPI calls; "
          f"{len(tweets)} posts after dropping replies", file=sys.stderr)
    ...
    report = {
        ...
        "replies_dropped": len(fetched) - len(tweets),
        "getxapi_calls": calls,
